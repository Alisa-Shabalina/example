...
std::vector<double> Recalculator::GetValuesByKey(Json::Value val, const int& key) {
	...
	for (int i = 0; i < val.size(); ++i) {
		double new_val = val[i][key].asDouble();
...
Json::Value Recalculator::Recalculate(const std::map<ValueName, std::vector<double>>& point) {
	...
	for (const auto& item : Values) {
		...
		switch (name) {
			...
			{
				//std::shared_ptr<ContiniousVariableValue> it = std::make_shared<ContiniousVariableValue>(item);
				...
				for (int i = 0; i < temp["data"].size(); ++i) {
					temp["data"][i][grad_key] =  (grad[i]);
					temp["data"][i][stress_key] =  (stress_min[i]);
				}
			...
			case ValueName::POISSON_RATIO: 
			{
				...
				for (int i = 0; i < temp["data"].size(); ++i) {
					temp["data"][i][poisson_key] =  (poisson[i]);
				}
			...
			case ValueName::FLUID_LOSS:
			{
				...
				for (int i = 0; i < temp["data"].size(); ++i) {
					temp["data"][i][loss_key] =  (fluid_loss[i]);
				}
			...
			case ValueName::YOUNG_MODULUS:
			{
				...
				for (int i = 0; i < temp["data"].size(); ++i) {
					temp["data"][i][young_key] =  (young_modulus[i]);
				}
			...
			case ValueName::SLURRY_VOLUME:
			{
				...
				if (item->GetExternalRelation() == ValueName::TIME) {
					...
					for (size_t i = 0; i < time.size(); ++i) {
						...
						if (slurry_vol[i] != 0.0 && slurry_rate[i] != 0.0) {
					...
					//std::cout << std::endl;
					//
					//std::cout << "Slurry volume after: \n";
					//for (size_t i = 0; i < slurry_vol.size(); ++i) {
					//	std::cout << slurry_vol[i] << ' ';
					//}
					//std::cout << std::endl;
					//
					//std::cout << "Slurry rate after: \n";
					//for (size_t i = 0; i < slurry_rate.size(); ++i) {
					//	std::cout << slurry_rate[i] << ' ';
					//}
					//std::cout << std::endl;

					for (int i = 0; i < temp["data"].size(); ++i) {
						temp["data"][i][s_v_key] =  (slurry_vol[i]);
						temp["data"][i][t_s_v_key] =  (t_slurry_vol[i]);
						temp["data"][i][time_key] =  (time[i]);
						temp["data"][i][t_time_key] =  (t_time[i]);
						temp["data"][i][f_v_key] =  (fluid_vol[i]);
						temp["data"][i][t_f_v_key] =  (t_fluid_vol[i]);
						temp["data"][i][f_m_key] =  (fluid_mass[i]);
					}
				}
				if (item->GetExternalRelation() == ValueName::SLURRY_RATE) {
					...
					for (int i = 0; i < temp["data"].size(); ++i) {
						temp["data"][i][s_v_key] =  (slurry_vol[i]);
						temp["data"][i][t_s_v_key] =  (t_slurry_vol[i]);
						temp["data"][i][s_r_key] =  (slurry_rate[i]);
						temp["data"][i][f_v_key] =  (fluid_vol[i]);
						temp["data"][i][t_f_v_key] =  (t_fluid_vol[i]);
						temp["data"][i][f_m_key] =  (fluid_mass[i]);
						temp["data"][i][f_r_key] =  (fluid_rate[i]);
					}
				}
			...
			case ValueName::PROPANT_CONCENTRATION:
			{
				...
				for (int i = 0; i < temp["data"].size(); ++i) {
					temp["data"][i][f_v_key] =  (fluid_vol[i]);
					temp["data"][i][p_v_key] =  (propant_vol[i]);
					temp["data"][i][p_c_s_key] =  (prop_con_s[i]);
					temp["data"][i][p_c_e_key] =  (prop_con_e[i]);
					temp["data"][i][p_m_key] =  (prop_mass[i]);
					temp["data"][i][f_m_key] =  (fluid_mass[i]);
					temp["data"][i][f_r_key] =  (fluid_rate[i]);
					temp["data"][i][p_r_key] =  (prop_rate[i]);
					temp["data"][i][t_f_v_key] =  (t_fluid_vol[i]);
					temp["data"][i][t_p_m_key] =  (t_prop_mass[i]);
				}
			...
			case ValueName::PROPANT_MASS:
			{
				...
				for (int i = 0; i < temp["data"].size(); ++i) {
					...
					std::string new_val_2 = temp["data"][i][p_t_key].asString();
					//std::cout << new_val_2 << '\n';
					if (new_val_2 == "" || new_val_2 == "0") empty_indexes.push_back(i);
					prop_type[i] = move(new_val_2);
				}
				...
				//for (int i = 0; i < propant_vol.size(); ++i) {
				//	std::cout << propant_vol[i] << ' ';
				//}
				//std::cout << std::endl;
				//
				//std::cout << "Proppant conc start: \n";
				//for (int i = 0; i < prop_con_s.size(); ++i) {
				//	std::cout << prop_con_s[i] << ' ';
				//}
				//std::cout << std::endl;
				//
				//std::cout << "Proppant conc end: \n";
				//for (int i = 0; i < prop_con_e.size(); ++i) {
				//	std::cout << prop_con_e[i] << ' ';
				//}
				//std::cout << std::endl;
			...
			case ValueName::FLUID_TYPE:
			{

				break;
			}

			case ValueName::PROPANT_TYPE:
			{
				...
				std::vector<std::string> fluid_type(fluid_vol.size()), prop_type(fluid_vol.size());
				// если несколько видв пропантов, то тут должен быть цикл какой-то сложный
				//std::cout << point.at(name)[0];
				auto smth = item->GetIdName();
				std::string new_prop = item->GetIdName().at(static_cast<int>(point.at(name)[0])); //Propant code
				std::vector<int> empty_indexes;
				for (int i = 0; i < temp["data"].size(); ++i) {
					std::string new_val_1 = temp["data"][i][f_t_key].asString();
					//std::cout << new_val_1 << '\n';
					...
					//std::cout << new_val_2 << '\n';
					if (new_val_2 == "" || new_val_2 == "0") empty_indexes.push_back(i);
					//если пропантов несколько, то тут посложнее все должно быть
					prop_type[i] = new_prop;
				}
					...
					for (size_t i = 0; i < prop_con_s.size(); ++i) {
						propant_vol[i] = .5 * slurry_vol[i] *
							(prop_con_s[i] / (p_densities[i] +
								prop_con_s[i]) + prop_con_e[i] / (p_densities[i] + prop_con_e[i]));
						prop_mass[i] = propant_vol[i] * p_densities[i];
						prop_rate[i] = propant_vol[i] / time[i];
 					}
					...
					propant_vol = ClearFields(propant_vol, empty_indexes);
					prop_mass = ClearFields(prop_mass, empty_indexes);
					prop_rate = ClearFields(prop_rate, empty_indexes);

					//clear prop_type fields 
					for (const auto& i : empty_indexes) {
						for (size_t j = 0; j < prop_type.size(); ++j) {
							if (i == j) prop_type[j] = "";
						}
					}
					...
					for (size_t i = 0; i < fluid_vol.size(); ++i) {
						...
						t_fluid_vol[i] = std::accumulate(fluid_vol.begin(), fluid_vol.begin() + i + 1, 0.);
						t_prop_mass[i] = std::accumulate(prop_mass.begin(), prop_mass.begin() + i + 1, 0.);
						fluid_rate[i] = fluid_vol[i] / time[i] - prop_rate[i];
						fluid_mass[i] = f_densities[i] * fluid_vol[i];
					}


					for (int i = 0; i < temp["data"].size(); ++i) {
						temp["data"][i][p_t_key] = (prop_type[i]);
						temp["data"][i][p_v_key] = (propant_vol[i]);
						temp["data"][i][p_m_key] = (prop_mass[i]);
						temp["data"][i][t_f_v_key] = (t_fluid_vol[i]);
						temp["data"][i][t_p_m_key] = (t_prop_mass[i]);
